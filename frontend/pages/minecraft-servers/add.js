...
export const getServerSideProps = async (context) => {
  ...
  if (query?.ip) {
    const res = await fetch(
      `${process.env.BACKEND_URI}/servers/pre-checks/${query.ip}`
    );
    ...
    return {
      props: {
        initialIp: query.ip,
        alreadyExists: data?.alreadyExists || null,
        existingServer: data?.existingServer || null,
        ping: data?.ping || null,
...
const AddAServer = ({ initialIp = "", existingServer, ping, error }) => {
  ...
  const onSubmit = async () => {
    ...
    const res = await fetch(`${process.env.BACKEND_URI}/servers`, {
      method: "POST",
      headers: {
